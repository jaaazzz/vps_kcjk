...
#洛杉矶-亚洲优化KVM
url1 = 'https://hostodo.com/portal/cart.php?gid=36'
#洛杉矶-KVM
url2 = 'https://hostodo.com/portal/cart.php?gid=34'
#迈阿密-KVM
url3 = 'https://hostodo.com/portal/cart.php?gid=35'
try:
    response1 = requests_pkg.get(url1)
    response2 = requests_pkg.get(url2)
    response3 = requests_pkg.get(url3)
    if response1 is None or response2 is None or response3 is None:
        email_sender.send_email('hostodo','没抓到网页')
    else:
        ...
        #print html_body
        searchObj = re.findall(r'<div class="field-row clearfix">(.*?)</div>',response_body, re.S|re.I)
        all_info = {}
        notice = ''
        for item in searchObj: 
            #print item
            name = re.search( r'<strong>(.*?)</strong>', item, re.S|re.I)
            have = item.find('0 Available')
            all_info[name.group(1).strip()] = have
        #print all_info

        sql1 = "select info from vps_update_info where provider = 'hostodo' Order by update_time desc limit 1"
        ...
        print 'notice='+notice
        if notice != '':
            email_sender.send_email('hostodo',notice)
        fld_inserttime = time.strftime(
            '%Y-%m-%d %H:%M:%S', time.localtime(int(time.time())))
        sql = "insert into vps_update_info (provider,info,update_time)values('hostodo',%s, %s )"
        vals = (json.dumps(all_info),fld_inserttime)
        db_oper.exe_insert(sql, vals)
except:
    email_sender.send_email('hostodo','start hostodo failed')